...
def execute_queries():
    ...
    if file_path:
        with open(file_path, 'rb') as file:
            response = requests.post('http://localhost:5000/execute_queries', files={'file': file})
        ...
        label_status.config(text="Please select a file.", fg="red")
...
button_execute.grid(row=1, column=1, padx=5, pady=5)

label_status = tk.Label(root, text="")
label_status.grid(row=2, column=1, padx=5, pady=5)
